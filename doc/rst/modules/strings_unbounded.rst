...
:doc:`strings` :doc:`strings_maps` :doc:`strings_fixed`
...
are represented by the subtype :vhdl:type:`~strings_unbounded.unbounded_string` which is derived from line
to ease interoperability with ``std.textio.line`` and :vhdl:type:`~strings_unbounded.unbounded_string` are of
